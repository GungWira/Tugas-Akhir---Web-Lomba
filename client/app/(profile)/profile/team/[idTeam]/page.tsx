...
export default function DetailLomba({
    ...
    return (
        <div className="relative bg-blueFade items-start w-full min-h-screen">
            <NavbarBackTitled href="/profile/team" className="pb-6 rounded-xl" style={{borderTopLeftRadius:'0', borderTopRightRadius: '0'}}>Team Detail</NavbarBackTitled>
            
            <div className="flex pt-24 min-h-screen gap-2 w-full">
                <div className="bg-[#F1F2F6] w-full min-h-full rounded-3xl px-4 py-8 flex flex-col justify-start items-start gap-2" onClick={() => handleCompetition()}>        
                    <div className="bg-white rounded-xl w-full p-4 py-6 flex flex-col gap-2">
                        <h1 className="text-normalText font-poppinsSemiBold text-lg">Nama Kompetisi</h1>
                        ...
                        <div className="flex flex-col justify-start items-start gap-4 w-full">
                            {loading ? 
                                ...
                                <div className="flex flex-row justify-start items-center gap-2">
                                    ...
                                    <p className="text-blueSec text-base font-poppinsMedium">Tim Siap</p>
                                </div>
                            }
                            <div className="w-full">
                                {user && user.imageUrl ? 
                                    <p className="font-poppinsSemiBold text-normalText text-lg">Tim maba</p>
                                    : <div className="w-3/5 bg-[#F1F2F6] text-[#F1F2F6] text-lg">p</div>
                                }
                            </div>
                        </div>
                        <div className="flex flex-col justify-start items-start gap-1 w-full">
                            <div className="w-full bg-white border px-3 py-1 rounded-md flex flex-row justify-between gap-2 items-center">
                                <div className="min-w-10 max-w-10 w-10 aspect-square rounded-full border overflow-hidden">
                                    <Image
                                        src={!loading ? user!.imageUrl : '/imgs/dashboard-imgs/Default-Profile-Img.svg'}
                                ...
                                <div className="w-full">
                                    {loading ? 
                                        <div className="font-poppinsMedium text-sm w-3/5 bg-[#F1F2F6] text-[#F1F2F6]">p</div> :
                                        <p className="text-normalText font-poppinsMedium text-sm">{user && user.name ? user.name : ''}</p>
                                    }
                                </div>
                                
                                <div className="w-full mt-1">
                                    {loading ? 
                                        <div className="font-poppinsMedium text-xs w-3/5 bg-[#F1F2F6] text-[#F1F2F6]">p</div> :
                            ...
                            <div className="w-full border px-3 py-1 rounded-md flex flex-row justify-between gap-2 items-center">
                                ...
                                <div className="w-full mt-1">
                                    {loading ? 
                                        <div className="font-poppinsMedium text-sm w-3/5 bg-[#F1F2F6] text-[#F1F2F6]">p</div> :
                                        <p className="text-normalText font-poppinsMedium text-sm">1 Slot Tersisa</p>
                                    }
                                </div>
                    ...
                    <div className="flex flex-col justify-start items-start w-full">
                        ...
                        <Button 
                            ...
                            style={{backgroundColor: `#F1F2F6`, color: `#767676`, marginTop: '12px'}}
                            onClick={() => {}}
                        >
                            Stop Unggahan Tim
                        </Button>
